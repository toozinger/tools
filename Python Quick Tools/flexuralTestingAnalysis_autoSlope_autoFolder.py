...
root = tk.Tk()
root.withdraw()
ctypes.windll.user32.MessageBoxW(0, "Select the folder where datafiles are stored. Sub-folders are OK", "Selection 1", 1)
dataFilePath = filedialog.askdirectory(title = "Select the folder of test data")
if not dataFilePath: sys.exit()

dataFiles = []
for subdir, dirs, files in os.walk(dataFilePath):
    for file in files:
...
def flexural(fileLocal, fileConstants, index):
    ...
    print(f"{iterations} optimization iterations performed")
    ...
    # Save fig
    fig1.savefig(f"{saveLocal}/{testName} load disp graph.png")
...
for index, fileLocal in enumerate(dataFiles):
...
# Save plots
fig1.savefig(f"{saveLocal}/Combined test data.png",bbox_inches="tight" )
fig2.savefig(f"{saveLocal}/strength plot.png", bbox_inches="tight")
fig3.savefig(f"{saveLocal}/Stiffness plot.png", bbox_inches="tight")
